...
// Definir el slug del archivo principal para las actualizaciones
...
class Proteccion_Anti_Bots {
    ...
    public function __construct() {
        // 1. Agregar enlaces en la lista de plugins (Ajustes, Soporte, Docs)
        add_filter( 'plugin_action_links_' . PROTECCION_ANTI_BOTS_FILE, array( $this, 'agregar_enlaces_plugin' ) );

        // 2. Permitir controlar actualizaciones automáticas desde la lista de plugins
        add_filter( 'plugin_auto_update_setting_html', array( $this, 'html_actualizacion_automatica' ), 10, 3 );

        // 3. Agregar menú de administración
        add_action( 'admin_menu', array( $this, 'agregar_menu_admin' ) );
        add_action( 'admin_init', array( $this, 'registrar_ajustes' ) );

        // 4. Alerta de seguridad (Usuario igual a Display Name)
        add_action( 'admin_notices', array( $this, 'alerta_usuario_publico' ) );

        // 5. Sistema de Actualizaciones
        $this->init_actualizaciones();

        // 6. Lógica de bloqueo (solo si el plugin está activado en ajustes)
    ...
    // ==============================================================================
    // AJUSTES VISUALES EN LA LISTA DE PLUGINS
    // ==============================================================================
    ...
    public function agregar_enlaces_plugin( $links ) {
        ...
        // Fusionar los nuevos enlaces con los existentes (Desactivar)
        return array_merge( $enlaces_ajustes, $links );
    }

    public function html_actualizacion_automatica( $html, $plugin_file, $plugin_data ) {
        // Solo mostrar para este plugin específico
        if ( $plugin_file !== PROTECCION_ANTI_BOTS_FILE ) {
            return $html;
        }
        
        // Devolver el HTML estándar de WordPress para el control de actualizaciones
        return $html;
    }
    ...
    public function init_actualizaciones() {
        ...
        // Slug del plugin (carpeta/archivo)
    ...
    public function check_for_update( $transient ) {
        ...
        if ( $remote_version ) {
            // Obtener versión local del archivo actual
            ...
            if ( version_compare( $local_version, $remote_version, '<' ) ) {
                $obj = new stdClass();
                $obj->slug = dirname( $this->plugin_slug ); // WordPress espera el nombre de la carpeta como slug en algunos casos
    ...
    public function plugin_info( $false, $action, $args ) {
        // Comprobar si la consulta es para este plugin
        // $args->slug suele ser el nombre de la carpeta
        $slug_plugin = dirname( $this->plugin_slug );
        
        if ( $args->slug !== $slug_plugin ) {
            return $false;
        }
        ...
        $info->sections = array(
            'description' => 'Plugin de seguridad personalizado para bloqueo de bots y corrección de usuarios vulnerables. Incluye protección 404 y auditoría de nombres de usuario.',
            'changelog' => '<h4>Versión ' . $remote_version . '</h4><ul><li>Mejoras en interfaz y enlaces de soporte.</li><li>Corrección de errores generales.</li></ul>',
            'author' => '<a href="https://github.com/xoreaxmrgamer">XorEax MrGamer</a>'
        );
    ...
    private function get_remote_version() {
        // Intenta obtener el archivo RAW
    ...
    public function agregar_menu_admin() {
        // NOTA: Cambié el 'menu_slug' a 'proteccion-anti-bots' para que coincida con el nuevo nombre
        add_options_page(
